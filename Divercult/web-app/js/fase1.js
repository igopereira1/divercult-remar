...
window.onload = function(){
   ...
   // addListeners();
    ...
    $('#save').click(function () {
        ...
        if ((params.split(",").length -1 == 5) || (params.split(",").length -1 == 7) || (params.split(",").length -1 == 9))  {
            params = params.substr(0, params.length -1);
            window.top.location.href = "/divercult/fase1/toJson/" + params;
        }
        else{
            Materialize.toast("Por favor, selecione cinco, sete ou nove pares de imagem/texto.", 2000);
        }
...
function _delete() {
    ...
    if(list_id.length<=0){
        alert("Você deve selecionar ao menos um par para excluir");
    }
    else{
        if(list_id.length==1){
            if(confirm("Você tem certeza que deseja deletar esse par?")){
                url = location.origin + '/divercult/fase1/delete/' + list_id[0];
                data = {_method: 'DELETE'};
                trID = "#tr"+list_id[0];
                $.ajax({
                        type: 'DELETE',
            ...
            if(confirm("Você tem certeza que deseja deletar esses pares?")){
                for(var i=0;i<list_id.length;i++){
                    url = location.origin + '/divercult/fase1/delete/' + list_id[i];
                    data = {_method: 'DELETE'};
                    trID = "#tr"+list_id[i];
                    $(trID).remove();
                    $.ajax({
